...
import { paperAccounts, userSettings } from "../../../../db/schema";
...
export async function POST(request: Request) {
  ...
  const [settings] = await getDb().select({ encrypted: userSettings.tickflowApiKeyEncrypted }).from(userSettings).where(eq(userSettings.userId, user.userId)).limit(1);
  const realtimeKey = settings?.encrypted ? await decryptSecret(settings.encrypted) : null;
  const quotes = realtimeKey
    ? await mapSettledWithConcurrency(symbols, 4, async (symbol) => {
      const quote = await fetchTickFlowQuote(symbol, realtimeKey);
      return { symbol, price: quote.lastPrice, date: shanghaiDate(new Date(quote.timestamp)) };
    })
    : await mapSettledWithConcurrency(symbols, 4, async (symbol) => {
  ...
  return Response.json({
    ...
    source: realtimeKey ? "realtime" : "daily",
  });
}
