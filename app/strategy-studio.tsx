...
function StrategyDocs({ strategies, onDeleted }: { strategies: Array<SavedStrategy & { builtin?: boolean }>; onDeleted: (id: string) => void }) {
  return <div className="strategy-docs">
    ...
    <div className="docs-list">{strategies.map((strategy) => <article key={strategy.id}><header><div><b>{strategy.name}</b><em>{strategy.builtin ? "内置" : strategy.tag} · v{strategy.version ?? 1}</em></div>{!strategy.builtin && <button onClick={() => onDeleted(strategy.id)}>删除</button>}</header><p>{strategy.description}</p><dl><div><dt>买入</dt><dd>{strategyRuleSummary(strategy.entryRules, strategy.entryLogic)}</dd></div><div><dt>卖出</dt><dd>{strategyRuleSummary(strategy.exitRules, strategy.exitLogic)}</dd></div></dl><small className="strategy-version-note">不可变规则快照{strategy.contentHash ? ` · ${strategy.contentHash.slice(0, 12)}` : " · 内置版本"}；模拟盘会保存所用版本。</small></article>)}</div>
