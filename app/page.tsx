...
type Strategy = SavedStrategy & {
  ...
  recommendedRisk?: { position: number; stopLoss: number; takeProfit: number };
};
...
const builtinStrategies: Strategy[] = [
  ...
  { id: "ccb-steady-trend", name: "建行低波稳健趋势", description: "基于建设银行上市以来前复权日K设计：只在长期趋势向上、短期波动收敛且RSI不过热时参与；建议60%仓位、8%止损、35%止盈。", summary: "MA20 多头 · 波动率 < 1.6 · RSI < 62", tag: "建行验证", color: "#78d7b6", builtin: true, horizon: "long", recommendedRisk: { position: 60, stopLoss: 8, takeProfit: 35 }, entryLogic: "and", exitLogic: "or", entryRules: [{ left: "close", operator: "gt", rightType: "indicator", rightIndicator: "ma20" }, { left: "ma20", operator: "gt", rightType: "indicator", rightIndicator: "ma60" }, { left: "volatility20", operator: "lt", rightType: "value", rightValue: 1.6 }, { left: "rsi14", operator: "lt", rightType: "value", rightValue: 62 }], exitRules: [{ left: "close", operator: "lt", rightType: "indicator", rightIndicator: "ma60" }] },
];
...
export default function Home() {
  ...
  const recommendedRiskStrategy = selectedStrategies.find((strategy) => strategy.recommendedRisk);
      ...
      <div className="workspace">
        ...
        <aside className="control-column">
          <section className="strategy-panel panel">
            ...
            <div className="parameters"><div className="section-label"><b>执行参数</b><button onClick={() => { setPosition(30); setStopLoss(8); setTakeProfit(22); }}>恢复默认</button></div>{recommendedRiskStrategy?.recommendedRisk && <button className="recommended-risk" onClick={() => { const risk = recommendedRiskStrategy.recommendedRisk!; setPosition(risk.position); setStopLoss(risk.stopLoss); setTakeProfit(risk.takeProfit); setToast(`已应用${recommendedRiskStrategy.name}的稳健参数`); }}><span><b>应用回测建议参数</b><small>{recommendedRiskStrategy.name}</small></span><em>{recommendedRiskStrategy.recommendedRisk.position}% 仓位 · {recommendedRiskStrategy.recommendedRisk.stopLoss}% 止损 · {recommendedRiskStrategy.recommendedRisk.takeProfit}% 止盈</em></button>}<label><span>单票仓位 <b>{position}%</b></span><input type="range" min="10" max="100" step="5" value={position} onChange={(event) => setPosition(Number(event.target.value))} /></label><div className="parameter-pair"><label><span>止损线</span><div><input type="number" min="1" max="20" value={stopLoss} onChange={(event) => setStopLoss(Number(event.target.value))} /><i>%</i></div></label><label><span>止盈线</span><div><input type="number" min="5" max="50" value={takeProfit} onChange={(event) => setTakeProfit(Number(event.target.value))} /><i>%</i></div></label></div><button className={`toggle-row ${autoRebalance ? "on" : ""}`} onClick={() => setAutoRebalance((value) => !value)} aria-pressed={autoRebalance}><span><b>收盘后自动调仓</b><small>新日K到达后检查信号</small></span><i><em /></i></button></div>
