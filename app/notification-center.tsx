...
import { defaultMessageTemplate, notificationContentTypes, notificationMethods, notificationTypeLabels, templateForNotificationType, type NotificationChannelType, type NotificationContentType, type NotificationHeader, type NotificationMethod } from "./notification-model";
import { formatAppDateTime } from "./timezone";
...
const variableNames = ["action", "actionText", "automationName", "paperAccountName", "stockName", "symbol", "strategyName", "price", "executionPrice", "shares", "simulatedShares", "quantityText", "tradeSummary", "reason", "dataMode", "source", "marketTime", "sentAt"];
...
export function NotificationCenter({ open, onClose, onToast }: { open: boolean; onClose: () => void; onToast: (message: string) => void }) {
  ...
  const resetForm = () => {
    setEditingId(null); setChannelType("webhook"); setName("我的 Webhook"); setUrl(""); setMethod("POST"); setContentType("application/json"); setHeaders([]); setHeadersDirty(false); setMessageTemplate(defaultMessageTemplate); setError("");
  };

  const editChannel = (channel: Channel) => {
    setEditingId(channel.id); setChannelType(channel.type); setName(channel.name); setUrl(channel.url); setMethod(channel.method); setContentType(channel.contentType); setMessageTemplate(channel.messageTemplate); setHeaders([]); setHeadersDirty(false); setError("");
  };

  const selectChannelType = (type: NotificationChannelType) => {
    setChannelType(type);
    setMethod("POST"); setContentType("application/json"); setHeaders([]); setHeadersDirty(type !== "webhook"); setMessageTemplate(templateForNotificationType(type));
    if (!editingId) setName(type === "dingtalk" ? "钉钉策略提醒" : type === "feishu" ? "飞书策略提醒" : "我的 Webhook");
  };
  ...
  const saveChannel = async () => {
    ...
    try {
      const body: Record<string, unknown> = { name, type: channelType, url, method, contentType, messageTemplate };
      if (channelType === "webhook" && (!editingId || headersDirty)) body.headers = headers;
  ...
  const testChannel = async (channel: Channel) => {
    ...
    try {
      ...
      onToast(`${notificationTypeLabels[channel.type]}测试消息已送达`); await refresh();
  ...
  return <div className="studio-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="strategy-studio automation-studio" role="dialog" aria-modal="true" aria-labelledby="notification-title">
      <header className="studio-header"><div><span>NOTIFICATION CHANNELS</span><h2 id="notification-title">通知</h2><p>独立配置各类通知渠道和消息模板，再由定时任务选择使用。</p></div><button onClick={onClose} aria-label="关闭">×</button></header>
      ...
      <div className="studio-body automation-body">
        ...
        {tab === "channels" && <div className="notification-config-layout">
          <section className="notification-config-form">
            ...
            <div className="notification-type-picker"><button className={channelType === "webhook" ? "selected webhook" : "webhook"} onClick={() => selectChannelType("webhook")}><i>⌁</i><span><b>自定义 Webhook</b><small>完整自定义请求</small></span></button><button className={channelType === "dingtalk" ? "selected dingtalk" : "dingtalk"} onClick={() => selectChannelType("dingtalk")}><i>钉</i><span><b>钉钉通知</b><small>群机器人</small></span></button><button className={channelType === "feishu" ? "selected feishu" : "feishu"} onClick={() => selectChannelType("feishu")}><i>飞</i><span><b>飞书通知</b><small>群机器人</small></span></button></div>
            <label><span>渠道名称</span><input value={name} maxLength={50} onChange={(event) => setName(event.target.value)} /></label>
            <label><span>{channelType === "dingtalk" ? "钉钉机器人 Webhook URL" : channelType === "feishu" ? "飞书机器人 Webhook URL" : "Webhook URL"}</span><input type="url" value={url} onChange={(event) => setUrl(event.target.value)} placeholder={channelType === "dingtalk" ? "https://oapi.dingtalk.com/robot/send?access_token=..." : channelType === "feishu" ? "https://open.feishu.cn/open-apis/bot/v2/hook/..." : "https://example.com/webhook"} /><small>{channelType === "webhook" ? "仅支持公网 HTTPS 地址。" : `从${channelType === "dingtalk" ? "钉钉" : "飞书"}群机器人设置中复制完整地址，系统固定使用 POST JSON。`}</small></label>
            {channelType === "webhook" ? <><div className="automation-form-grid"><label><span>请求方法</span><select value={method} onChange={(event) => setMethod(event.target.value as NotificationMethod)}>{notificationMethods.map((item) => <option value={item} key={item}>{item}</option>)}</select></label><label><span>Content-Type</span><select value={contentType} onChange={(event) => setContentType(event.target.value as NotificationContentType)}>{notificationContentTypes.map((item) => <option value={item} key={item}>{item}</option>)}</select></label></div><div className="headers-editor"><div><span>自定义请求头</span><button onClick={() => { setHeadersDirty(true); setHeaders((current) => [...current, { name: "", value: "" }]); }}>＋ 添加</button></div>{editingId && channels.find((channel) => channel.id === editingId)?.headerNames.length ? <small>已保存：{channels.find((channel) => channel.id === editingId)?.headerNames.join("、")}。请求头值已加密，不会回显；不添加新列表则保留原配置。</small> : null}{headers.map((header, index) => <div className="header-row" key={index}><input value={header.name} onChange={(event) => updateHeader(index, "name", event.target.value)} placeholder="Header 名称" /><input type="password" autoComplete="off" value={header.value} onChange={(event) => updateHeader(index, "value", event.target.value)} placeholder="Header 值" /><button aria-label="删除请求头" onClick={() => { setHeadersDirty(true); setHeaders((current) => current.filter((_, itemIndex) => itemIndex !== index)); }}>×</button></div>)}</div></> : <div className={`preset-request-note ${channelType}`}><i>{channelType === "dingtalk" ? "钉" : "飞"}</i><span><b>已使用{channelType === "dingtalk" ? "钉钉" : "飞书"}文本消息格式</b><small>POST · application/json · 无需额外请求头</small></span></div>}
          ...
          <section className="automation-list-wrap">
            <div className="automation-section-title"><div><span>CHANNELS</span><h3>已配置渠道</h3></div><button onClick={refresh}>{loading ? "刷新中…" : "刷新"}</button></div>
            <div className="channel-list">{channels.length ? channels.map((channel) => <article className={`${!channel.enabled ? "disabled" : ""} channel-${channel.type}`} key={channel.id}><header><div><i className={channel.enabled ? "ready" : ""} /><b>{channel.name}</b><em>{notificationTypeLabels[channel.type]}</em></div><button className={`task-toggle ${channel.enabled ? "on" : ""}`} onClick={() => toggleChannel(channel)} aria-label={channel.enabled ? "停用渠道" : "启用渠道"}><span /></button></header><p title={channel.url}>{compactUrl(channel.url)}</p><dl><div><dt>请求</dt><dd>{channel.method}</dd></div><div><dt>Content-Type</dt><dd>{channel.contentType}</dd></div><div><dt>请求头</dt><dd>{channel.headerNames.length ? channel.headerNames.join("、") : "无"}</dd></div><div><dt>最近测试</dt><dd>{channel.lastTestAt ? formatAppDateTime(channel.lastTestAt) : "尚未测试"}</dd></div></dl><footer><button disabled={testingId === channel.id || !channel.enabled} onClick={() => testChannel(channel)}>{testingId === channel.id ? "测试中…" : "测试连通性"}</button><button onClick={() => editChannel(channel)}>编辑</button><button className="danger" onClick={() => deleteChannel(channel)}>删除</button></footer></article>) : <div className="automation-empty"><i>⌁</i><b>还没有通知渠道</b><p>在左侧创建 Webhook、钉钉或飞书通知。</p></div>}</div>
          </section>
        </div>}
        {tab === "logs" && <div className="notification-history"><div className="automation-section-title"><div><span>DELIVERY LOG · ASIA/SHANGHAI</span><h3>通知投递记录</h3></div><button onClick={refresh}>{loading ? "刷新中…" : "刷新"}</button></div>{logs.length ? <div className="notification-table"><div className="notification-head"><span>北京时间</span><span>类型</span><span>渠道 / 标的</span><span>原因</span><span>投递</span></div>{logs.map((log) => <div className="notification-row" key={log.id}><time>{formatAppDateTime(log.createdAt)}</time><b className={log.type}>{log.type === "buy" ? "买入" : log.type === "sell" ? "卖出" : log.type === "test" ? "测试" : "异常"}</b><span>{channels.find((channel) => channel.id === log.notificationChannelId)?.name ?? "—"}<small>{log.symbol}{log.price ? ` · ¥${log.price.toFixed(2)}` : ""}</small></span><p>{log.reason}</p><em className={log.status}>{log.status === "success" ? "已送达" : log.httpStatus ? `HTTP ${log.httpStatus}` : "失败"}</em></div>)}</div> : <div className="automation-empty"><i>✓</i><b>暂无投递记录</b><p>测试渠道或任务触发信号后会显示在这里。</p></div>}</div>}
      </div>
    </section>
  </div>;
}
