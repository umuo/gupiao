...
function normalizeQuote(quote: RawTickFlowQuote): TickFlowQuote | null {
  const lastPrice = Number(quote.last_price);
  if (!Number.isFinite(lastPrice) || lastPrice <= 0 || !quote.symbol) return null;
...
export async function fetchTickFlowQuotes(symbols: string[], apiKey: string): Promise<TickFlowQuote[]> {
  ...
  try {
    ...
    const payload = await response.json() as { data?: RawTickFlowQuote[] };
    const quotes = (payload.data ?? []).map(normalizeQuote).filter((quote): quote is TickFlowQuote => Boolean(quote));
    if (!quotes.length) throw new Error("TickFlow 未返回有效实时行情");
    return quotes;
  } finally {
...
export async function fetchTickFlowQuote(symbol: string, apiKey: string): Promise<TickFlowQuote> {
  const quotes = await fetchTickFlowQuotes([symbol], apiKey);
