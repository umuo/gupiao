...
type ChannelSummary = { id: string; name: string; enabled: boolean };
type Automation = {
  id: string;
  name: string;
  symbol: string;
...
export function TaskCenter({ open, watchlist, strategies, defaultStopLoss, defaultTakeProfit, onClose, onToast, onOpenNotifications }: {
  ...
  const effectiveChannelId = enabledChannels.some((channel) => channel.id === channelId) ? channelId : (enabledChannels[0]?.id ?? "");
  ...
  const saveTask = async () => {
    ...
    if (!effectiveChannelId) { setError("请先在通知模块创建并启用一个通知渠道"); return; }
    if (dataMode === "realtime" && !tickflow.configured) { setTab("data"); setError("实时任务需要先配置 TickFlow Key"); return; }
    setSaving(true); setError("");
    try {
      const response = await fetch("/api/automations", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name, symbol: toSymbol(selectedStock.code), stockName: selectedStock.name, strategyId: selectedStrategy.id, strategy: selectedStrategy, notificationChannelId: effectiveChannelId, dataMode, runTime, intervalMinutes, stopLoss, takeProfit }),
      });
  ...
  const runTask = async (id: string) => {
    ...
    try {
      ...
      onToast(payload.result?.action ? `已发送${payload.result.action === "buy" ? "买入" : "卖出"}提醒` : payload.result?.reason || "检查完成");
      await refresh();
    } catch (caught) { setError(caught instanceof Error ? caught.message : "任务检查失败"); }
  };

  const patchTask = async (item: Automation, changes: { enabled?: boolean; resetPosition?: boolean }) => {
      ...
      <div className="studio-body automation-body">
        ...
        {tab === "tasks" && <div className="automation-layout">
          <section className="automation-form">
            ...
            <label><span>通知渠道</span>{enabledChannels.length ? <select value={effectiveChannelId} onChange={(event) => setChannelId(event.target.value)}>{enabledChannels.map((channel) => <option key={channel.id} value={channel.id}>{channel.name}</option>)}</select> : <button className="empty-channel-link" onClick={onOpenNotifications}>＋ 前往通知模块创建渠道</button>}<small>任务只保存渠道关联，Webhook 细节在通知模块统一修改。</small></label>
            ...
            <button className="primary-studio-button automation-save" onClick={saveTask} disabled={saving || !effectiveChannelId || !name}>{saving ? "正在创建…" : "创建并启用任务"}</button>
          </section>
          <section className="automation-list-wrap">
            ...
            <div className="automation-list">{items.length ? items.map((item) => <article className={!item.enabled ? "disabled" : ""} key={item.id}><header><div><i className={item.dataMode} /> <b>{item.name}</b></div><button className={`task-toggle ${item.enabled ? "on" : ""}`} onClick={() => patchTask(item, { enabled: !item.enabled })} aria-label={item.enabled ? "停用任务" : "启用任务"}><span /></button></header><p><strong>{item.stockName}</strong><span>{item.symbol}</span><em>{item.strategyName}</em></p><dl><div><dt>数据</dt><dd>{item.dataMode === "realtime" ? `实时 · ${item.intervalMinutes}分钟` : `日K · ${item.runTime}`}</dd></div><div><dt>模拟状态</dt><dd>{item.positionState === "holding" ? "持仓" : "空仓"}</dd></div><div><dt>通知渠道</dt><dd>{item.notificationChannelName}</dd></div><div><dt>最近通知</dt><dd>{item.lastNotifiedAt ? new Date(item.lastNotifiedAt).toLocaleString("zh-CN") : "尚未触发"}</dd></div></dl><footer><button onClick={() => runTask(item.id)}>立即检查</button>{item.positionState === "holding" && <button onClick={() => patchTask(item, { resetPosition: true })}>重置为空仓</button>}<button className="danger" onClick={() => deleteTask(item)}>删除</button></footer></article>) : <div className="automation-empty"><i>◴</i><b>还没有定时任务</b><p>选择股票、策略和通知渠道后创建。</p></div>}</div>
          </section>
        </div>}
