...
export function TaskCenter({ open, watchlist, strategies, defaultStopLoss, defaultTakeProfit, onClose, onToast, onOpenNotifications }: {
  open: boolean;
  ...
  const saveTask = async () => {
    ...
    try {
      const response = await fetch("/api/automations", {
        ...
        body: JSON.stringify({ name, symbol: toSymbol(selectedStock.code), stockName: selectedStock.name, strategyId: selectedStrategy.id, strategy: selectedStrategy, notificationChannelIds: selectedChannelIds, dataMode, runTime, intervalMinutes, stopLoss, takeProfit }),
      });
      ...
      await refresh(); onToast("定时策略任务已创建");
  ...
  const runTask = async (id: string) => {
    ...
    try {
      ...
      const payload = await response.json() as { error?: string; result?: { action: string | null; reason: string; deliveries?: { succeeded: number; total: number } } };
      if (!response.ok) throw new Error(payload.error || "任务检查失败");
      onToast(payload.result?.action ? `已向 ${payload.result.deliveries?.succeeded ?? 0}/${payload.result.deliveries?.total ?? 0} 个渠道发送${payload.result.action === "buy" ? "买入" : "卖出"}提醒` : payload.result?.reason || "检查完成");
      await refresh();
    } catch (caught) { setError(caught instanceof Error ? caught.message : "任务检查失败"); }
  };

  const patchTask = async (item: Automation, changes: { enabled?: boolean; resetPosition?: boolean; notificationChannelIds?: string[] }) => {
    ...
    setEditingTaskChannels(null); await refresh();
  };
  ...
  return <div className="studio-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="strategy-studio automation-studio" role="dialog" aria-modal="true" aria-labelledby="task-title">
      <header className="studio-header"><div><span>SCHEDULED TASKS</span><h2 id="task-title">定时任务</h2><p>只管理策略、行情和执行时间；通知方式由通知模块独立维护。</p></div><button onClick={onClose} aria-label="关闭">×</button></header>
      ...
      <div className="studio-body automation-body">
        ...
        {tab === "tasks" && <div className="automation-layout">
          <section className="automation-form">
            <div className="automation-section-title"><div><span>NEW TASK</span><h3>新建策略任务</h3></div><small>不会执行真实委托</small></div>
            ...
            {dataMode === "daily" ? <label><span>每天检查时间（北京时间）</span><input type="time" value={runTime} onChange={(event) => setRunTime(event.target.value)} /><small>工作日执行；09:35 默认使用上一根已完成日K。</small></label> : <label><span>交易时段检查间隔</span><select value={intervalMinutes} onChange={(event) => setIntervalMinutes(Number(event.target.value))}>{[1, 5, 15, 30, 60].map((value) => <option key={value} value={value}>每 {value} 分钟</option>)}</select><small>仅 09:30–11:30、13:00–15:00 检查。</small></label>}
            ...
            <button className="primary-studio-button automation-save" onClick={saveTask} disabled={saving || !selectedChannelIds.length || !name}>{saving ? "正在创建…" : `创建任务 · ${selectedChannelIds.length} 个渠道`}</button>
          </section>
          <section className="automation-list-wrap">
            ...
            <div className="automation-list">{items.length ? items.map((item) => <article className={!item.enabled ? "disabled" : ""} key={item.id}><header><div><i className={item.dataMode} /> <b>{item.name}</b></div><button className={`task-toggle ${item.enabled ? "on" : ""}`} onClick={() => patchTask(item, { enabled: !item.enabled })} aria-label={item.enabled ? "停用任务" : "启用任务"}><span /></button></header><p><strong>{item.stockName}</strong><span>{item.symbol}</span><em>{item.strategyName}</em></p><dl><div><dt>数据</dt><dd>{item.dataMode === "realtime" ? `实时 · ${item.intervalMinutes}分钟` : `日K · ${item.runTime}`}</dd></div><div><dt>模拟状态</dt><dd>{item.positionState === "holding" ? "持仓" : "空仓"}</dd></div><div><dt>通知渠道</dt><dd>{item.notificationChannelNames.length ? item.notificationChannelNames.join("、") : "未配置"}</dd></div><div><dt>最近通知</dt><dd>{item.lastNotifiedAt ? new Date(item.lastNotifiedAt).toLocaleString("zh-CN") : "尚未触发"}</dd></div></dl>{editingTaskChannels === item.id && <div className="task-card-channel-editor"><span>调整通知渠道</span><div>{enabledChannels.map((channel) => { const selected = taskChannelDraft.includes(channel.id); return <button className={selected ? "selected" : ""} key={channel.id} onClick={() => setTaskChannelDraft((current) => selected ? current.filter((id) => id !== channel.id) : [...current, channel.id])}>{selected ? "✓ " : ""}{channel.name}</button>; })}</div><footer><button onClick={() => setEditingTaskChannels(null)}>取消</button><button className="save" disabled={!taskChannelDraft.length} onClick={() => patchTask(item, { notificationChannelIds: taskChannelDraft })}>保存渠道</button></footer></div>}<footer><button onClick={() => runTask(item.id)}>立即检查</button><button onClick={() => { setEditingTaskChannels(item.id); setTaskChannelDraft(item.notificationChannelIds.filter((id) => enabledChannels.some((channel) => channel.id === id))); }}>设置渠道</button>{item.positionState === "holding" && <button onClick={() => patchTask(item, { resetPosition: true })}>重置为空仓</button>}<button className="danger" onClick={() => deleteTask(item)}>删除</button></footer></article>) : <div className="automation-empty"><i>◴</i><b>还没有定时任务</b><p>选择股票、策略和通知渠道后创建。</p></div>}</div>
          </section>
        </div>}
