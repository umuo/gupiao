...
export function TaskCenter({ open, watchlist, strategies, defaultStopLoss, defaultTakeProfit, targetAccount, onClose, onToast, onOpenNotifications, onTaskChanged }: {
  ...
  const [strategyIds, setStrategyIds] = useState<string[]>(strategies[0] ? [strategies[0].id] : []);
  ...
  const effectiveStrategies = targetAccount ? [...targetAccount.strategies, ...strategies.filter((strategy) => !targetAccount.strategies.some((snapshot) => snapshot.id === strategy.id))] : strategies;
  ...
  const effectiveStrategyIds = targetAccount ? targetAccount.strategies.map((strategy) => strategy.id) : strategyIds.filter((id) => effectiveStrategies.some((strategy) => strategy.id === id));
  ...
  const selectedStrategies = effectiveStrategyIds.flatMap((id) => {
    const strategy = effectiveStrategies.find((item) => item.id === id);
  ...
  useEffect(() => {
    ...
    setStrategyIds(targetAccount.strategies.map((strategy) => strategy.id));
  ...
  const saveTask = async () => {
    if (!selectedStock || !selectedStrategies.length) { setError("请先选择股票和策略"); return; }
    ...
    try {
      ...
      const response = await fetch("/api/automations", {
        ...
        body: JSON.stringify(taskId
          ...
          : { name, paperAccountId: targetAccount?.id, symbol: toSymbol(selectedStock.code), stockName: selectedStock.name, strategyId: selectedStrategies[0].id, strategies: selectedStrategies, notificationChannelIds: selectedChannelIds, dataMode, runTime, intervalMinutes, stopLoss, takeProfit }),
      });
      ...
      <div className="studio-body automation-body">
        ...
        {tab === "tasks" && <div className="automation-layout">
          <section className="automation-form">
            ...
            {targetAccount && <div className="task-account-lock"><i>◎</i><span><b>{targetAccount.name}</b><small>{targetAccount.stockName} · {targetAccount.strategies.length} 个策略同时生效</small></span><em>关联模拟盘</em></div>}
            <label><span>任务名称</span><input value={name} maxLength={50} onChange={(event) => setName(event.target.value)} /></label>
            {!targetAccount && <><label><span>自选股</span><select value={effectiveStockCode} onChange={(event) => setStockCode(event.target.value)}>{watchlist.map((stock) => <option key={stock.code} value={stock.code}>{stock.name} · {stock.code}</option>)}</select></label><fieldset className="task-strategy-picker"><legend>执行策略 <em>{selectedStrategies.length} / 8</em></legend><small>任一策略产生信号即可执行。</small><div>{effectiveStrategies.map((strategy) => { const selected = effectiveStrategyIds.includes(strategy.id); return <button type="button" aria-pressed={selected} className={selected ? "selected" : ""} key={strategy.id} onClick={() => toggleStrategy(strategy.id)}><i>{selected ? "✓" : ""}</i>{strategy.name}</button>; })}</div></fieldset></>}
          ...
          <section className="automation-list-wrap">
            ...
            <div className="automation-list">{items.length ? items.map((item) => <article className={!item.enabled ? "disabled" : ""} key={item.id}><header><div><i className={item.dataMode} /> <b>{item.name}</b></div><button className={`task-toggle ${item.enabled ? "on" : ""}`} onClick={() => patchTask(item, { enabled: !item.enabled })} aria-label={item.enabled ? "停用任务" : "启用任务"}><span /></button></header>{item.paperAccountName && <div className="task-linked-account">◎ {item.paperAccountName}</div>}<p><strong>{item.stockName}</strong><span>{item.symbol}</span><em>{item.strategies?.map((strategy) => strategy.name).join("、") || item.strategyName} · {item.strategies?.length || 1} 个策略</em></p><dl><div><dt>数据</dt><dd>{item.dataMode === "realtime" ? `实时 · ${item.intervalMinutes}分钟` : `日K · ${item.runTime}`} · {item.timezone}</dd></div><div><dt>最近运行</dt><dd className={`run-state ${item.lastStatus}`}>{item.lastStatus === "succeeded" ? "检查完成" : item.lastStatus === "failed" ? `失败 · ${item.lastError || "查看记录"}` : item.lastStatus === "retrying" ? "自动重试中" : "等待运行"}</dd></div><div><dt>通知渠道</dt><dd>{item.notificationChannelNames.length ? item.notificationChannelNames.join("、") : "未配置"}</dd></div><div><dt>最近通知</dt><dd>{item.lastNotifiedAt ? formatAppDateTime(item.lastNotifiedAt) : "尚未触发"}</dd></div></dl>{editingTaskChannels === item.id && <div className="task-card-channel-editor"><span>调整通知渠道</span><div>{enabledChannels.map((channel) => { const selected = taskChannelDraft.includes(channel.id); return <button className={selected ? "selected" : ""} key={channel.id} onClick={() => setTaskChannelDraft((current) => selected ? current.filter((id) => id !== channel.id) : [...current, channel.id])}>{selected ? "✓ " : ""}{channel.name}</button>; })}</div><footer><button onClick={() => setEditingTaskChannels(null)}>取消</button><button className="save" disabled={!taskChannelDraft.length} onClick={() => patchTask(item, { notificationChannelIds: taskChannelDraft })}>保存渠道</button></footer></div>}<footer><button onClick={() => runTask(item.id)}>立即检查</button><button onClick={() => { setEditingTaskChannels(item.id); setTaskChannelDraft(item.notificationChannelIds.filter((id) => enabledChannels.some((channel) => channel.id === id))); }}>设置渠道</button>{item.positionState === "holding" && !item.paperAccountId && <button onClick={() => patchTask(item, { resetPosition: true })}>重置为空仓</button>}<button className="danger" onClick={() => deleteTask(item)}>删除</button></footer></article>) : <div className="automation-empty"><i>◴</i><b>还没有定时任务</b><p>选择股票、策略和通知渠道后创建。</p></div>}</div>
          </section>
        </div>}
        {tab === "runs" && <div className="automation-run-history"><div className="automation-section-title"><div><span>EXECUTION LEDGER · ASIA/SHANGHAI</span><h3>任务运行记录</h3></div><button onClick={() => refreshRuns(false)}>{loading ? "刷新中…" : "刷新"}</button></div>{runs.length ? <div className="automation-run-list">{runs.map((run) => <article key={run.id}><time>{formatAppDateTime(run.createdAt)}</time><i className={run.status === "succeeded" && run.action && run.deliveryTotal > 0 && !run.deliverySucceeded ? "failed" : run.status}>{run.status === "succeeded" ? run.action ? run.deliveryTotal && !run.deliverySucceeded ? "通知失败" : "已触发" : "已检查" : run.status === "failed" ? "失败" : run.status === "retrying" ? "重试" : run.status === "running" ? "运行" : run.status === "skipped" ? "跳过" : "排队"}</i><span><b>{run.automation?.name ?? "任务已删除"}</b><small>{run.automation ? `${run.automation.stockName} · ${run.automation.symbol}` : run.trigger === "manual" ? "手动触发" : "定时触发"}</small></span><p>{run.action ? `${run.action === "buy" ? "买入信号" : "卖出信号"}${run.price ? ` · ¥${run.price.toFixed(3)}` : ""} · ` : ""}{run.error || run.reason || "等待执行"}</p><em>{run.status === "retrying" ? `${run.attempt}/${run.maxAttempts} · ${run.nextRetryAt ? formatAppDateTime(run.nextRetryAt) : "待重试"}` : run.action ? run.deliveryTotal ? `通知 ${run.deliverySucceeded}/${run.deliveryTotal}` : "通知未发送" : "未触发通知"}</em></article>)}</div> : <div className="automation-empty"><i>✓</i><b>暂无运行记录</b><p>任务定时触发或手动检查后会显示在这里。</p></div>}{runCursor && <button className="load-more-runs" disabled={loading} onClick={() => refreshRuns(true, runCursor)}>{loading ? "加载中…" : "加载更多"}</button>}</div>}
